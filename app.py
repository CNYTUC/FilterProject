...
from strateji_xml import (
    ...
    strateji_sil, xml_icerik_goster
)
...
def _init():
    defaults = {
        ...
        "strateji_adi": "", "bildirim": None, "bildirim_tip": "info",
    }
...
# Text input widget'i render edilmeden önce senkronizasyon yapılmalı
if st.session_state.get("input_sync_pending", False):
    st.session_state["strateji_adi_input"] = st.session_state.get("strateji_adi", "")
    st.session_state["input_sync_pending"] = False
elif not st.session_state.get("strateji_adi_input") and st.session_state.get("strateji_adi"):
    st.session_state["strateji_adi_input"] = st.session_state.get("strateji_adi", "")
...
with st.sidebar:
    ...
    # ── Strateji Adı ─────────────────────────────────────────────────────────
    st.text_input("Strateji Adı", placeholder="Örn: Stratejim 1", key="strateji_adi")
    ...
    with b2:
        if st.button("➕ Ekle", use_container_width=True, help="Yeni strateji olarak kaydet"):
            ad = st.session_state["strateji_adi"].strip()
...
strat_adi_goster = (st.session_state.get("strateji_adi") or "—")[:14]
